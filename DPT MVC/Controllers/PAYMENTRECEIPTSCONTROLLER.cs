...
namespace DPT.MVC.Controllers
{
    public class PAYMENTRECEIPTSCONTROLLER : Controller
    {
        ...
        public PAYMENTRECEIPTSCONTROLLER(ILogger<PAYMENTRECEIPTSCONTROLLER> logger, IConfiguration configuration, IHttpClientFactory httpClientFactory)
        {
            _logger = logger;
        ...
        [HttpPost]
        public async Task<ActionResult> SavePAYMENTRECEIPTS(PAYMENTRECEIPTS model)
        {
            ...
            try
            {
                if (model != null)
                {
                    ...
                    if (model.ID != 0)
                    {
                        ...
                        requestBody = new HttpRequestMessage(HttpMethod.Patch, "/api/PAYMENTRECEIPTS");
                    ...
                    else
                    {
                        ...
                        model.LASTUPDATED = DateTime.Now;
                        model.LASTUPDATEDBY = Convert.ToInt32(HttpContext.Session.GetString("UserId"));
                        requestBody = new HttpRequestMessage(HttpMethod.Post, "/api/PAYMENTRECEIPTS");
                    }
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        int requesthdrId = data["paymentreceipts"]["id"].Value<int>();
        ...
        public async Task<JsonResult> GetAllPAYMENTRECEIPTS()
        {
            try
            {
                var client = _httpClientFactory.CreateClient("DPTClient");
